...
const BASE_URL = 'http://localhost:8080/tools.descartes.teastore.webui'; // <- Change this if needed
...
export default function () {
    ...
    check(addExtraToCartRes, { 'extra cart 302': (r) => r.status === 302 });
    ...
    check(removeExtraRes, { 'remove product 302': (r) => r.status === 302 });

    let cartRes3 = http.get(`${BASE_URL}/cart`);
    check(cartRes3, { 'final cart view 200': (r) => r.status === 200 });

    let proceedCheckoutRes = http.post(`${BASE_URL}/cartAction`, {
        productid: '55',
        orderitem_7: '24',
        proceedtoCheckout: 'Proceed to Checkout',
    });
    check(proceedCheckoutRes, { 'proceed to checkout 302': (r) => r.status === 302 });

    let orderPage = http.get(`${BASE_URL}/order`);
    check(orderPage, { 'order page 200': (r) => r.status === 200 });
    ...
    check(orderSubmit, { 'submit order 302': (r) => r.status === 302 });
